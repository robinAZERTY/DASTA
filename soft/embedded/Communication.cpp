...
String Communication::describe()
{
    ...
    description += String(X->size)+"*"+String(s1v)+";";
    }
    if(send_P){
    description += "P" + String(TYPE_MATRIX);
    description += String(P->rows)+"*"+String(P->cols)+"*"+String(s1v)+";";
    }
    if(send_Z){
    description += "Z" + String(TYPE_VECTOR);
    description += String(Z->size)+"*"+String(s1v)+";";
    }
    if(send_h){
    description += "h" + String(TYPE_VECTOR);
    description += String(h->size)+"*"+String(s1v)+";";
    }
    if(send_internal_events){
    description += "event" + String(TYPE_STRING);
    }
