...
def draw_box(img):
    ...
    for vertex in vertices:
        projected.append(calibration.project(vertex,np.array(received_orientation),np.array([0,0,0]),np.array([-0.3,0,0]),np.array([ 0.5, 0.5, 0.5, 0.5 ]),300))
...
if __name__ == "__main__":
    ...
    threading.Thread(target=draw_attitude_ForEver).start()
