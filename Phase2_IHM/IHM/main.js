...
function animate() {
  ...
  if (droneModel) {
    ...
    // droneModel.rotation.x = 0;
    // droneModel.rotation.y = 0;
    // droneModel.rotation.z = 0;
  }
...
socket.onmessage = function (event) {
  ...
  const new_orientation_x = data.orientation_rpy[0];
  const new_orientation_y = data.orientation_rpy[1];
  const new_orientation_z = data.orientation_rpy[2];

  target_orientation_x = new_orientation_x;
  target_orientation_y = new_orientation_y;
  target_orientation_z = new_orientation_z;

  console.log('RPY', data.orientation_rpy);
};
